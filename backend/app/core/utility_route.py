...
def database_health(db: Database = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Database query execution failed after successful connection: {
                str(e)}",
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"An unexpected error occurred during database health check logic: {
                str(e)}",
        )
...
# Warning: Below this message are routes that would not be implemented on
# the real site, but are instead used for testing and showing
# implementation
