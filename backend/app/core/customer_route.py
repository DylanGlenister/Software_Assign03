...
def get_customer_account(
    token: Optional[str] = Depends(get_token), db: Database = Depends(get_db)
) -> dict[str, CustomerAccount | str | None]:
    if token:
...
def register_route(payload: RegisterPayload, db: Database = Depends(get_db)):
    ...
    account: CustomerAccount = CustomerAccount.register(
        db, payload.email, payload.password, Role.CUSTOMER
    )
    ...
    return {"message": "Registration successful", "email": account.email}


@customer_route.get("/trolley")
def get_trolley_route(
    customer_data: dict[str, CustomerAccount | str | None] = Depends(
...
def add_to_trolley_route(
    item: TrollyItem, customer_data: dict = Depends(get_customer_account)
):
...
def remove_from_trolley_route(
    ...
    success: bool = bool(customer.trolley.remove_from_trolley(item.product_id))
    if success:
...
def customer_get_orders(
    ...
    result: dict = {"token": customer_data.get("token")}
    result["orders"] = db.get_orders_from_account(
        customer.accountID
    )  # an array of orders
...
def customer_create_order(
    payload: AddressID, customer_data: dict = Depends(get_customer_account)
):
    customer: CustomerAccount | None = customer_data.get("account")
...
def customer_add_address(
    payload: Address, customer_data: dict = Depends(get_customer_account)
):
    customer: CustomerAccount | None = customer_data.get("account")
