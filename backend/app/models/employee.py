...
class TagCreate(BaseModel):
    name: str = Field(
        ..., min_length=1, max_length=100, description="The name of the tag."
    )
...
class EmployeeAccount(Account):
    ...
    def create_product_in_catalogue(
        self, product_data: ProductCreate, catalogue_service: Catalogue
    ) -> Product:
        """
        ...
        try:
            ...
            raise HTTPException(
                status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="An unexpected error occurred during product creation.",
            )

    def update_product_in_catalogue(
            self,
            product_id: ID,
            product_data: ProductUpdate,
            catalogue_service: Catalogue) -> Product:
        """
        Handles the business logic for updating an existing product via the catalogue service.
        """
        updated_product = catalogue_service.update_product(
    ...
    def assign_tag_to_product(
        ...
        try:
            ...
            raise HTTPException(
                status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to add tag to product: {str(e)}",
            )

    def remove_tag_from_a_product(
            self, product_id: ID, tag_id: ID) -> dict[str, str]:
        """Removes a tag association from a product."""
        try:
            self.db.remove_tag_from_product(product_id, tag_id)
            return {
                "message": f"Tag {tag_id} removed from product {product_id} successfully."}
        except ValueError as e:
            raise HTTPException(
            ...
            raise HTTPException(
                status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to remove tag from product: {str(e)}",
            )

    def add_image_url_to_product(
        self, product_id: ID, image_url: str
        ...
        try:
            ...
            raise HTTPException(
                status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to add image to product: {str(e)}",
            )
